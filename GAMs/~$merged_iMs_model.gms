...
eq_cAct_calc(sample, gene) .. cAct_calc(sample, gene) =e= sum(iM, cAct_mapping(gene, iM) * (
...
eq_cInh_calc(sample, gene) .. cInh_calc(sample, gene) =e= sum(iM, cInh_mapping(gene, iM) * (
...
* objective equations
act_obj1 .. act_diff1 =e= sum((gene, sample), (abs((cAct_calc(sample, gene) -
                                               sum(iM, cAct(gene, iM, sample)))) / max_cAct(gene) )**2);

inh_obj1 .. inh_diff1 =e= sum((gene, sample), (abs((cInh_calc(sample, gene) -
                                               sum(iM, cInh(gene, iM, sample)))) / max_cInh(gene) )**2);

match_obj .. match_diff =e= sum((gene, sample), (abs(actual_mRNA(sample, gene) -
                                ((cAct_calc(sample, gene)*basal_constants('KdRNAP', gene) +
                                basal_constants('KdRNAPCrp', gene))*(basal_constants('KdRNAP', gene) +
                                sample_constants('RNAP', sample) +
                                basal_constants('KeqOpening', gene)*sample_constants('RNAP', sample)))
                                / (((1 + cAct_calc(sample, gene) +
                                cInh_calc(sample, gene))*basal_constants('KdRNAP', gene)*basal_constants('KdRNAPCrp', gene) +
                                cAct_calc(sample, gene)*basal_constants('KdRNAP', gene)*(1 + basal_constants('KeqOpening', gene))*sample_constants('RNAP', sample) + basal_constants('KdRNAPCrp', gene)*(1 + basal_constants('KeqOpening', gene))*sample_constants('RNAP', sample)))))**2 );
