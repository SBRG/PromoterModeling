...
act_metab_Total.lo(sample) = log10(input_parameters('act_metab_Total_lo'));
act_metab_Total.up(sample) = log10(input_parameters('act_metab_Total_up'));
...
act_metab_Total.l(sample) = uniformInt(-10, -3);
...
eq_cInh_calc(sample, gene) .. cInh_calc(sample, gene) =e= 10**inh_TF_conc(sample) / 10**inh_Kd(gene);

eq_cAct_calc(sample, gene) .. cAct_calc(sample, gene) =e= 3 * (10**act_metab_Total(sample)) * 10**act_Kd(gene) + input_constants('kd_act_metab') * 10**act_metab_Total(sample) + 3 * (10**act_metab_Total(sample)) * meas_act_TF(sample) + (-36*(10**act_metab_Total(sample)) * (10**act_Kd(gene))**2 * meas_act_TF(sample) + (3 * (10**act_metab_Total(sample)) * 10**act_Kd(gene) + input_constants('kd_act_metab') * 10**act_Kd(gene) + 3 * (10**act_Kd(gene) * meas_act_TF(sample)))**2)**(.5) / (18 * (10**act_Kd(gene))**2);
